<#
Report Automation for Pending Reboot Reports
Erick Sevilla
#>
        ...
        foreach ($Computer in $ComputerName) {
            try {
                ...
                if ($WMI_Reg) {
                    ...
                    if ($WMI_Reg.GetMultiStringValue($HKLM,"SYSTEM\CurrentControlSet\Control\Session Manager\",'PendingFileRenameOperations').sValue) {$PendingReboot = $false;$PFROperations = 'True'}
